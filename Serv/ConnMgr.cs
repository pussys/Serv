...
public partial class ConnMgr
{
    ...
    public void MsgLogin(Conn conn, ProtocolBase protoBase)
    {
        ...
        PlayerNumber playerNumber = DataMgr.instance.GetPlayerNumber(id);
        if (playerData == null&& playerNumber == null)
        {
            protocolRet.AddInt(-1);
            conn.Send(protocolRet);
            return;
        ...
        conn.player.number = playerNumber;
        //事件触发
        ServNet.instance.playerEvent.OnLogin(conn.player);
