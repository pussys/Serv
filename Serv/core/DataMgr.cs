...
public class DataMgr
{
    ...
    public bool CreatePlayer(string id)
    {
        ...
        try
        {
            ...
            formatter.Serialize(stream1, playerNumber);
        }
        catch (Exception e)
        {
        ...
        byte[] byteArr1 = stream1.ToArray();
        //写入数据库
        string cmdStr = string.Format("insert into player set id ='{0}' ,data =@data;", id);
        string cmdnum = string.Format("insert into number set id ='{0}' ,number =@number;", id);
        MySqlCommand cmd = new MySqlCommand(cmdStr, sqlConn);
        MySqlCommand cmd1 = new MySqlCommand(cmdnum, sqlConn);
        cmd.Parameters.Add("@data", MySqlDbType.Blob);
        cmd.Parameters[0].Value = byteArr;
        cmd1.Parameters.Add("@number", MySqlDbType.Blob);
        cmd1.Parameters[0].Value = byteArr1;
        try
        {
            cmd.ExecuteNonQuery();
    ...
    //获取玩家数据
    public PlayerNumber GetPlayerNumber(string id)
    {
        ...
        if (!IsSafeStr(id))
            return playerNumber;
        //查询
        string cmdStr = string.Format("select * from number where id ='{0}';", id);
        MySqlCommand cmd = new MySqlCommand(cmdStr, sqlConn);
        byte[] buffer;
        try
        {
        ...
        {
            Console.WriteLine("[DataMgr]GetPlayerNumber 查询 " + e.Message);
            return playerNumber;
        }
        //反序列化
        MemoryStream stream = new MemoryStream(buffer);
        ...
        {
            BinaryFormatter formatter = new BinaryFormatter();
            playerNumber = (PlayerNumber)formatter.Deserialize(stream);
            return playerNumber;
        }
        catch (SerializationException e)
        {
            Console.WriteLine("[DataMgr]GetPlayerNumber 反序列化 " + e.Message);
            return playerNumber;
        }
    }
    ...
    public bool SavePlayerNumber(Player player)
    {
        string id = player.id;
        PlayerNumber playerNumber = player.number;
        //序列化
        IFormatter formatter = new BinaryFormatter();
        MemoryStream stream = new MemoryStream();
        try
        {
            formatter.Serialize(stream, playerNumber);
        }
        catch (Exception e)
        {
            Console.WriteLine("[DataMgr]SavePlayer 序列化 " + e.Message);
        ...
        byte[] byteArr = stream.ToArray();
        //写入数据库
        string formatStr = "update number set number =@number where id = '{0}';";
        string cmdStr = string.Format(formatStr, player.id);
        MySqlCommand cmd = new MySqlCommand(cmdStr, sqlConn);
        cmd.Parameters.Add("@number", MySqlDbType.Blob);
        cmd.Parameters[0].Value = byteArr;
        try
        {
            cmd.ExecuteNonQuery();
        ...
        {
            Console.WriteLine("[DataMgr]CreatePlayerNumber 写入 " + e.Message);
            return false;
        }
    }
}
