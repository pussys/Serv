...
public class ServNet
{
    ...
    //消息分发
    public ConnMgr ConnMgr = new ConnMgr();
    public LandMgr landMgr = new LandMgr();
    public PlayerEvent  playerEvent = new PlayerEvent();
    ...
    private void HandleMsg(Conn conn, ProtocolBase protoBase)
    {
        ...
        {
            MethodInfo mm = ConnMgr.GetType().GetMethod(methodName);
            if (mm == null)
            {
            ...
            mm.Invoke(ConnMgr, obj);
        }
        //角色协议分发
        else
        {
            MethodInfo mm = landMgr.GetType().GetMethod(methodName);
            if (mm == null)
            {
